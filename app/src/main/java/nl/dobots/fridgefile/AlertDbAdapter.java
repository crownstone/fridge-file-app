...
public class AlertDbAdapter {
	...
	// table name
	public static final String TABLE_NAME = "alert_log";
	...
	/**
	 * Create a new entry using the date and alert provided. If the entry is
	 * successfully created return the new rowId for that entry, otherwise return
	 * a -1 to indicate failure.
	 *
	 * @param date the date of the entry
	 * @param alert the alert for the entry
	 * @return rowId or -1 if failed
	 */
	public long createEntry(Date date, String alert) {
		ContentValues values = new ContentValues();

		values.put(KEY_DATETIME, date.getTime());
		values.put(KEY_ALERT, alert);
	 ...
	 */
	public boolean updateEntry(long id, Date date, String alert) {
		ContentValues values = new ContentValues();
	...
	public Cursor fetchAllEntries() {
		Cursor mCursor = mDb.query(TABLE_NAME, new String[] {KEY_ROWID, KEY_DATETIME, KEY_ALERT},
	 ...
	 */
	public Cursor fetchEntriesForDate(String date) throws ParseException {
		Date parsedDate = sdf_date.parse(date);
		return fetchEntriesForDate(parsedDate);
	}
	 ...
	 * @return cursor to access the entries
	 */
	public Cursor fetchEntriesForDate(Date date) {
		...
		return fetchEntriesForDateRange(startDate, endDate);
	}
	 ...
	 * @throws ParseException if a wrong date string is provided
	 */
	public Cursor fetchEntriesForDateRange(String startDate, String endDate) throws ParseException {
		Date parsedStartDate = sdf_date.parse(startDate);
		Date parsedEndDate = sdf_date.parse(endDate);
		return fetchEntriesForDateRange(parsedStartDate, parsedEndDate);
	}
	 ...
	 * @throws ParseException if a wrong date string is provided
	 */
	public Cursor fetchEntriesForDateRange(Date startDate, Date endDate) {
		String[] args = new String[] { String.valueOf(startDate.getTime()), String.valueOf(endDate.getTime()) };
		Cursor mCursor = mDb.query(TABLE_NAME, new String[] {KEY_ROWID, KEY_DATETIME, KEY_ALERT},
				KEY_DATETIME + " between ? and ?", args, null, null, null);
		if (mCursor != null) {
			mCursor.moveToFirst();
		}
		return mCursor;
	 ...
	 */
	public Cursor fetchEntriesForTime(String time) throws ParseException {
		Date parsedTime = sdf_time.parse(time);
		return fetchEntriesForTime(parsedTime);
	}
	 ...
	 * @return cursor to access the entries
	 */
	public Cursor fetchEntriesForTime(Date time) {
		Cursor mCursor = mDb.query(TABLE_NAME, new String[] {KEY_ROWID, KEY_DATETIME, KEY_ALERT},
				KEY_DATETIME + "=" + time.getTime(), null, null, null, null);
		if (mCursor != null) {
			mCursor.moveToFirst();
		}
		return mCursor;
	 ...
	 * @throws ParseException if a wrong time string is provided
	 */
	public Cursor fetchEntriesForTimeRange(String startTime, String endTime) throws ParseException {
		Date parsedStartTime = sdf_time.parse(startTime);
		Date parsedEndTime = sdf_time.parse(endTime);
		return fetchEntriesForTimeRange(parsedStartTime, parsedEndTime);
	}
	 ...
	 * @throws ParseException if a wrong time string is provided
	 */
	public Cursor fetchEntriesForTimeRange(Date startTime, Date endTime) {
		String[] args = new String[] { String.valueOf(startTime.getTime()), String.valueOf(endTime.getTime()) };
		Cursor mCursor = mDb.query(TABLE_NAME, new String[] {KEY_ROWID, KEY_DATETIME, KEY_ALERT},
				KEY_DATETIME + "between ? and ?", args, null, null, null);
		if (mCursor != null) {
			mCursor.moveToFirst();
